...
class Appointments_Model extends CI_Model {
    ...
    public function get_batch($where_clause = '') {
        $this->db->select('ea_appointments.id as id, book_datetime, start_datetime, end_datetime, notes, hash, is_unavailable, id_users_provider, id_users_customer, id_services, id_google_calendar,
ea_clinics.id as clinic_id, cl_clinic_name, cl_address_line_1, cl_address_line_2, cl_postcode, cl_phone_number, cl_email_address, cl_description');
        if ($where_clause != '') {
            $this->db->where($where_clause);
        }
        //$this->db->join('ea_clinics','ea_clinics.id=clinic_id');
        $this->db->join('ea_clinics','ea_clinics.id=clinic_id','left');        
