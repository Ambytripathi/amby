...
class Release_model extends Base_module_model {
	...
	public $foreign_keys = array('author_id' => 'teams_model'); // map foreign keys to table models
	...
	{
		parent::__construct('press'); // table name
	}

	function list_items($limit = NULL, $offset = NULL, $col = 'name', $order = 'asc', $just_count = FALSE)
	{
		$this->db->join('teams', 'teams.id = press.author_id', 'left');
		$this->db->select('press.id, press.title, summary, SUBSTRING(content, 1, 50) AS content, teams.name AS author, date_added, press.published', FALSE);
	...
	{	
		...
		$fields['content']['img_folder'] = 'images/press/';
		$fields['image']['folder'] = 'images/press/';
		$fields['thumb_image']['folder'] = 'images/press/thumbs/';
...
class Releas_model extends Base_module_record {
	
	public function get_url()
	{
		return site_url('images/press/'.$this->slug);
	}

	public function get_image_path()
	{
		return img_path('images/press/'.$this->image);
	}

	public function get_thumb_image_path()
	{
		return img_path('images/press/'.$this->thumb_image);
	}
}