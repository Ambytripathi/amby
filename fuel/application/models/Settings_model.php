...
class Settings_Model extends CI_Model {
     ...
     */
    public function get_setting($name, $clinic_id = NULL){
        ...
        if($clinic_id!=NULL){
            ...
            $query = $this->db->get_where('ea_settings', array('name' => $name, 'clinic_id' => $clinic_id));
        }else{
     ...
     */
    public function save_settings($settings, $clinic_id) {
        ...
        foreach($settings as $setting) {
            if($setting['name']=='company_working_plan'){
                ...
                $exists = $this->db->get('ea_settings')->result_array();
                ...
                if(sizeof($exists)>0){
                    ...
                    $this->db->where('name', $setting['name']);
                    $this->db->where('clinic_id', $clinic_id);
                    if (!$this->db->insert('ea_settings', array('value' => $setting['value'], 'clinic_id'=>$clinic_id, 'name'=>'company_working_plan'))) {
                        throw new Exception('Could not save setting (' . $setting['name']
                            . ' - ' . $setting['value'] . ')');
                    }
                }
     ...
     */
    public function get_settings($clinic_id = NULL) {
        if($clinic_id == NULL){
